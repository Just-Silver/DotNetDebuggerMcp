...
public class ReferenceExtractorTests
{
    ...
    public void DecompileCache_泛型实例化字段_收集内部类型()
    {
        ...
        var result = Extract(scope.Reader, "DotNetDebuggerMcp.Caching.DecompileCache");
        Assert.Contains("DotNetDebuggerMcp.Caching.CacheKey", result);
        Assert.Contains("DotNetDebuggerMcp.Caching.DecompileCache+CacheEntry", result);
    }
    ...
    public void ToolPipeline_方法返回与字段_收集内部类型()
    {
        ...
        using var scope = new MetadataScope(AssemblyPath);
        var result = Extract(scope.Reader, "DotNetDebuggerMcp.Pipeline.ToolPipeline");
        Assert.Contains("DotNetDebuggerMcp.Pipeline.ToolPipelineResult", result);
        Assert.Contains("DotNetDebuggerMcp.Caching.DecompileCache", result);
        Assert.Contains("DotNetDebuggerMcp.Caching.CacheKey", result);
    }
    ...
    public void WithExternal_内部集合与缺省API一致_外部集合含跨程序集类型()
    {
        ...
        using var scope = new MetadataScope(AssemblyPath);
        var (internalSet, external) = ExtractWithExternal(scope.Reader, "DotNetDebuggerMcp.Pipeline.ToolPipeline");
        var defaultResult = Extract(scope.Reader, "DotNetDebuggerMcp.Pipeline.ToolPipeline");

        Assert.Equal(defaultResult, internalSet);
        Assert.Contains("DotNetDebuggerMcp.Pipeline.ToolPipelineResult", internalSet);
        Assert.Contains("DotNetDebuggerMcp.Caching.DecompileCache", internalSet);
