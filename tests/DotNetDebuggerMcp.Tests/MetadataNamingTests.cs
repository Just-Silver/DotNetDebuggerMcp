...
public class MetadataNamingTests
{
    ...
    public void FullName_顶层类型_命名空间加类名()
    {
        ...
        Assert.Equal("DotNetDebuggerMcp.Formatting.OutputFormatter", MetadataNaming.FullName(reader, type));
    }
    ...
    public void FullName_嵌套类型_加号连接()
    {
        ...
        var handle = MetadataNaming.FindType(reader, "DotNetDebuggerMcp.Caching.DecompileCache+CacheEntry");
        Assert.True(handle.HasValue);
    }
    ...
    public void FindType_点号与加号分隔均可定位嵌套类型()
    {
        ...
        var plus = MetadataNaming.FindType(reader, "DotNetDebuggerMcp.Caching.DecompileCache+CacheEntry");
        var dot = MetadataNaming.FindType(reader, "DotNetDebuggerMcp.Caching.DecompileCache.CacheEntry");
    ...
    [Theory]
    [InlineData("class DotNetDebuggerMcp.Formatting.OutputFormatter")]
    [InlineData("Class DotNetDebuggerMcp.Formatting.OutputFormatter")] // 前缀大小写不敏感
    [InlineData("struct DotNetDebuggerMcp.Pipeline.ToolPipelineResult")]
    [InlineData("enum DotNetDebuggerMcp.Pipeline.DecompileKind")]
    ...
    public void FindTypes_返回全部归一化候选()
    {
        ...
        var candidates = MetadataNaming.FindTypes(reader, $"{TestDataPaths.SamplesNamespace}.Container.Inner");
        var viaFindType = MetadataNaming.FindType(reader, $"{TestDataPaths.SamplesNamespace}.Container.Inner");
    ...
    public void BuildNotFoundMessage_未找到时附相近类型名()
    {
        ...
        Assert.Contains($"{TestDataPaths.SamplesNamespace}.BigClass", message);
    }
    ...
    public void BuildAmbiguityMessage_头部与候选行格式()
    {
        ...
        var bigClass = MetadataNaming.FindType(reader, $"{TestDataPaths.SamplesNamespace}.BigClass")!.Value;
        var class0001 = MetadataNaming.FindType(reader, $"{TestDataPaths.SamplesNamespace}.Class0001")!.Value;
        ...
        Assert.Contains($"  {TestDataPaths.SamplesNamespace}.BigClass（token 0x{MetadataTokens.GetToken(bigClass):x8}）", message);
        Assert.Contains($"  {TestDataPaths.SamplesNamespace}.Class0001（token 0x{MetadataTokens.GetToken(class0001):x8}）", message);
    }
