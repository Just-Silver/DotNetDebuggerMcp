...
public class TypeListerTests
{
    ...
    public void 主程序集_列出struct_含ToolPipelineResult()
    {
        WithReader(MainAssemblyPath, reader =>
        {
            ...
            Assert.Equal('s', entries["DotNetDebuggerMcp.Pipeline.ToolPipelineResult"]);
        });
    }
    ...
    {
        // 主程序集原无 enum/delegate，进程内改造后新增 DotNetDebuggerMcp.Pipeline.DecompileKind 枚举，验证类别字母 e
        WithReader(MainAssemblyPath, reader =>
        {
            var entries = TypeLister.ListTypes(reader, "e").ToDictionary(e => e.FullName, e => e.Category);

            Assert.Equal('e', entries["DotNetDebuggerMcp.Pipeline.DecompileKind"]);
        });
    }
