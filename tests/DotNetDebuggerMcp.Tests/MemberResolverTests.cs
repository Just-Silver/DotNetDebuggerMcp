...
public class MemberResolverTests
{
    ...
    {
        var (typeFound, matches, _) = MemberResolver.FindMembers(AssemblyPath, "DotNetDebuggerMcp.Formatting.OutputFormatter", "Format");
    ...
    {
        var (typeFound, matches, _) = MemberResolver.FindMembers(AssemblyPath, "DotNetDebuggerMcp.Formatting.OutputFormatter", "");
    ...
    {
        var (typeFound, matches, _) = MemberResolver.FindMembers(AssemblyPath, "DotNetDebuggerMcp.Formatting.OutputFormatter", "ZzzNoSuch");
    ...
    public void FindMembers_默认排除访问器_includeAccessors为true时包含()
    {
        ...
        var (typeFound, matches, _) = MemberResolver.FindMembers(AssemblyPath, "DotNetDebuggerMcp.Pipeline.ToolCommand", "get");

        Assert.True(typeFound);
        Assert.DoesNotContain(matches, m => m.Name.StartsWith("get_"));

        var (_, withAccessors, _) = MemberResolver.FindMembers(AssemblyPath, "DotNetDebuggerMcp.Pipeline.ToolCommand", "get", includeAccessors: true);
    ...
    public void FindMembers_无匹配_返回相近成员名()
    {
        ...
        var (typeFound, matches, similar) = MemberResolver.FindMembers(AssemblyPath, "DotNetDebuggerMcp.Formatting.OutputFormatter", "FormatZz");
    ...
    public void FindMembers_嵌套类型_加号与点分隔均可定位()
    {
        ...
        var (plusFound, _, _) = MemberResolver.FindMembers(AssemblyPath, "DotNetDebuggerMcp.Caching.DecompileCache+CacheEntry", "");
        var (dotFound, _, _) = MemberResolver.FindMembers(AssemblyPath, "DotNetDebuggerMcp.Caching.DecompileCache.CacheEntry", "");
