...
/// <summary>
/// FieldAccessScanner 字段读写点扫描用例。
/// 素材：生成测试程序集（tests/TestData）中的 FieldHolder（public int Data;）——FieldUser.Read 读取 Data（ldfld）、
/// FieldWriter.Write 写入 Data（stfld）、无取地址（ldflda/ldsflda）。
/// </summary>
public class FieldAccessScannerTests
{
    ...
    public void FieldHolder_Data_读取写入定位且无取地址()
    {
        ...
        Assert.Contains(result.Reads, r => r.StartsWith("ILSpyMcp.Samples.FieldUser::"));
        Assert.Contains(result.Writes, w => w.StartsWith("ILSpyMcp.Samples.FieldWriter::"));
