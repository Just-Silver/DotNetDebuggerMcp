...
- **`debug_set` 停点改值工具（W1 现场改写）**：进程停在断点/异常后，把局部变量/参数/对象字段/数组元素当场改成新值，返回「原值 → 新值」回显再 `debug_continue` 观察行为——「把 X 改成 Y 再跑看是否复现/消失」的二分定位闭环。`path` 与 `debug_evaluate` 同款路径文法（根=栈顶帧局部/参数名 + 字段/下标）；`value` 支持 `null`（引用置空）/ `true`/`false` / 数字（整数/小数/后缀，按目标类型转换）/ 同帧另一条对象路径（引用重定向）。对象内 struct 字段、引用重定向均经 spike 实测支持；enum 对象字段直写 v1 给中文降级提示。不做：改 readonly/const/静态字段（元数据前置拒绝）、构造新对象、改字符串内容。注意风险：写目标进程内存可能使其崩溃——工具说明与返回文案均明示，只写读链路可定位的目标
