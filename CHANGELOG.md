...
- **UI 自动化主动触发业务操作（U1，`ui_find` / `ui_invoke` / `ui_wait` / `ui_scroll` 四件套）**：新增宿主级 FlaUI（UIA3）封装 + 四个 `ui_*` 工具，agent 可**真实操作运行中的 .NET UI 目标**（WPF/WinForms/…）并可与 `debug_*` 编排成「点按钮→断点命中→看现场」闭环。`ui_find` 按进程（pid/进程名）+窗口/控件条件返回**无视觉文本清单**（index/Name/Type/AutoId/Rect/Invoke✓），并对命中控件 Name/AutomationId 做**同名成员语义反查**（元数据旁路、按程序集缓存，命中附「类型全名.成员」候选、可用 `decompile_member` 看实现）；`ui_invoke` 真实点击（`action=click` 默认 InvokePattern 优先 + 物理左键兜底 / `rightClick` / `doubleClick`，后两者无 UIA pattern 一律物理鼠标），返回注明实际动作；`ui_wait` 只读轮询等控件文本出现或从 X 变 Y（超时返回当前状态不报错）；`ui_scroll` v1 物理滚轮在容器上滚动（direction=up/down + lines）。**不需活动 debug 会话**（可先操作 UI 再 attach）；全部操作串行 + UIA 跨进程 5s 双层超时护栏（目标无响应不挂死）；ui_invoke/ui_scroll 是**真实产线操作**——全程写入 AgentActionLog 供复盘，物理右键/双击/滚轮可能触发系统级行为、且在断开/非交互桌面会被系统拒绝（需交互桌面会话）
