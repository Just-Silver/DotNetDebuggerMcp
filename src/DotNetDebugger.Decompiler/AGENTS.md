...
- `Configuration/`（`DotNetDebugger.Decompiler.Configuration`）— 库自用内部常量，**自宿主 AppConfig/AppText 拆出的最小集，避免反向依赖宿主**：
  - `DecompilerConfig.cs`：`ExternalExpandMaxDepth`=5、`ExternalExpandMaxNodes`=200（call_chain 跨程序集展开预算）、`MaxOutputBytes`=64MB（单次反编译文本字符上限，超限返回「建议改用 decompile_to_dir」、结果不入缓存；与宿主缓存上限同值系设计一致而非同源）、`DependencyStreamOptions`=PrefetchMetadata（**引用程序集解析器（UniversalAssemblyResolver）打开依赖程序集必须带此选项**——构造时读完元数据即关流；缺省会留下无人 Dispose 的 PEFile 句柄持有到 GC，锁住被分析目录 dll 导致 dotnet build/clean 报 MSB3061/MSB3021，回归护栏 `AssemblyFileHandleTests`）
  - `DecompilerText.cs`：`DecompileFailurePrefix`=「反编译失败：」+ `StartsWithDecompileFailure` 判定、`IlFailurePrefix`=「反汇编失败：」+ `StartsWithIlFailure` 判定（DecompileIl 错误统一前缀）——**新增错误提示时检查是否需扩展判定，否则会被管道误当正常结果写缓存**（见下 InProcessDecompiler.IsErrorResult）
...
- `Decompiler/InProcessDecompiler.cs` — **唯一进程内反编译入口**，`public sealed class` 全静态：
  - `DecompileType` / `DecompileMember(assembly, token)` / `DecompileIl(assembly, token)`（方法体 IL 反汇编，MethodBodyDisassembler）/ `DecompileWholeModule` / `DecompileToDir` / `DecompileToProject`
  ...
  - `internal IsErrorResult(string)`：判定结果是错误提示而非反编译结果（覆盖「反编译失败：」/「反编译已取消」/「未找到类型 」/「反编译输出超过上限」/「元数据 token 」/「反汇编失败：」/引号开头七类）——**新增错误前缀必须同步扩展**，否则管道会把它写进缓存。
