...
/// GAC，解析优先级由 UniversalAssemblyResolver 内置），在该程序集定位被调方法并扫描其方法体子序列，
/// 子序列内的跨程序集调用递归展开（行随深度缩进）。纯元数据读取，不加载程序集、不反编译 IL；解析失败/找不到/防环 均返回空列表（调用方标注终止）。解码中止计数跨展开过程累计。
/// </summary>
public sealed class ExternalCallExpander : IDisposable
{
    ...
    /// <summary>
    /// 以主 dll 绝对路径构造展开器（内部自开 PEReader：resolver 需 mainAssemblyFileName 文件路径，PEReader 拿不到文件名； 目标框架经
    /// <see cref="DotNetCorePathFinderExtensions.DetectTargetFrameworkId(MetadataReader, string)"/> 探测）。
    /// </summary>
    ...
    /// <summary>
    /// 在类型定义内按成员名 + 参数个数定位方法定义：参数个数匹配优先（区分重载）；无参数个数匹配时取首个同名 （参数个数为 -1 时的兜底）；无同名方法返回 null。
    /// </summary>
