...
/// <summary>
/// TestData 测试程序集共享入口：自动发现 tests/TestData 目录下的 dll，并提供端到端用例共用的类型/成员标识，
/// 避免 Program 与各 Case 文件重复硬编码路径、类型名与成员 ID。
/// </summary>
public static class TestDataHelper
{
    /// <summary>仓库根目录（含 ILSpyMcp.slnx），随仓库整体移动自动适配。</summary>
    public static string RepoRoot { get; } = FindRepoRoot();

    /// <summary>测试程序集路径：自动选取 tests/TestData 下第一个 .dll。</summary>
    public static string Dll { get; } = Directory.GetFiles(Path.Combine(RepoRoot, "tests", "TestData"), "*.dll")
        .OrderBy(f => f, StringComparer.OrdinalIgnoreCase)
        .First();

    /// <summary>超大类型（反编译 600+ 行），可触发截断/分页/越界验证。</summary>
    public const string TypeName = "ILSpyMcp.Samples.BigClass";

    /// <summary>list_types 默认只返回前 200 行，此处用一个排在最前、必然可见的 class 类型名。</summary>
