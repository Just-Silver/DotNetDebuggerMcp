...
public static class InterfaceUsageCases
{
    public static IReadOnlyList<ToolCallCase> All(string dll) => new[]
    {
        ...
        new ToolCallCase("interface_usage", "IWorker 实现者段含 WorkerBase",
            new Dictionary<string, object?> { ["assembly"] = dll, ["typeName"] = $"{TestDataHelper.SamplesNamespace}.IWorker" },
            ExpectedContains: $"{TestDataHelper.SamplesNamespace}.WorkerBase", MustNotContain: "at System"),
        ...
        new ToolCallCase("interface_usage", "IWorker includeIndirect=true 含间接实现者 WorkerDerived",
            ...
            ExpectedContains: $"{TestDataHelper.SamplesNamespace}.WorkerDerived", MustNotContain: "at System"),
        ...
        new ToolCallCase("interface_usage", "IAnimal 调用点段含 AnimalCaller::Run → Speak",
            ...
            ExpectedContains: $"{TestDataHelper.SamplesNamespace}.AnimalCaller::Run → Speak", MustNotContain: "at System"),
        // 类型不存在应返回中文提示而非异常堆栈
