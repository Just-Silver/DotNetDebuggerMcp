...
public static class ListTypesTool
{
    ...
    /// <param name="categories">实体类型类别组合：c=class, i=interface, s=struct, d=delegate, e=enum，可组合如 "csi"（必填）。</param>
    ...
    public static Task<string> ListTypes(
        ...
        [Description("实体类型类别：c=class, i=interface, s=struct, d=delegate, e=enum；可组合多个字母，如 csi（必填）")] string categories = "",
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        // 参数校验：categories 必填且只能由 c/i/s/d/e 组成
        if (!ArgumentValidators.ValidateList(categories, out var argError)) return Task.FromResult(argError);
        ...
        var target = $"实体类别 {DescribeCategories(categories)}";
        if (!string.IsNullOrEmpty(nameContains)) target += $"，名称含 {nameContains}";
        if (!string.IsNullOrEmpty(namespaceContains)) target += $"，命名空间含 {namespaceContains}";
        var context = new FormatContext(assemblyFull, target, IsListing: true);

        // 元数据读取经共享缓存（命中直接返回，头部标注缓存命中）
        var signature = $"{CacheSignatures.ListTypes}{CacheSignatures.Separator}{categories}{CacheSignatures.Separator}{nameContains}{CacheSignatures.Separator}{namespaceContains}";
        return Task.FromResult(ToolExecutor.RunMetadataPe(assemblyFull, signature, lines, context, (pe, reader) =>
        {
            var typeList = TypeLister.ListTypes(reader, categories,
                string.IsNullOrEmpty(nameContains) ? null : nameContains,
                string.IsNullOrEmpty(namespaceContains) ? null : namespaceContains);
