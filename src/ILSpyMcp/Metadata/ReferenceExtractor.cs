...
/// TypeDefinition 集合（泛型实例化归约到定义，跨程序集 TypeReference 天然排除），供 dependencies 工具使用。只关注成员签名，
/// 不含基类/接口/特性——继承关系由 hierarchy 组件覆盖。
/// </summary>
public static class ReferenceExtractor
{
    ...
    public static IReadOnlyList<string> ExtractMemberSignatureReferences(MetadataReader reader, TypeDefinition type)
    {
        ...
        var provider = new Provider(reader, collected);
    ...
    /// <summary>
    /// 解析事件类型句柄并收集内部 TypeDefinition：TypeDefinition 直接收集；TypeSpecification（泛型实例化/数组等）走签名解码
    /// 触发回调；TypeReference 是外部类型，不收集。
    /// </summary>
    ...
    /// GetTypeFromDefinition 负责收集（本程序集类型），GetTypeFromReference 不收集（外部类型），TypeSpecification 递归解码，
