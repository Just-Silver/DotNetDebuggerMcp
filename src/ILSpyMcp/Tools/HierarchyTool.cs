...
public static class HierarchyTool
{
    ...
    [McpServerTool]
    [Description("查询 .NET 程序集（dll/exe）中指定类型的继承/接口关系：输出基类链（上溯到 System.Object）、类型实现的接口、以及程序集内直接继承它或实现其接口的类型。typeName 为类型全名，格式与 list_types 输出一致，可直接复制使用。结果默认只返回前约 8 KB，可用 lines 参数按行号范围拉取后续。")]
