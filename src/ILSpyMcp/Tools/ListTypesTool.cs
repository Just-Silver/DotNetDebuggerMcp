...
public static class ListTypesTool
{
    ...
    /// <summary>
    /// 列出指定类别的实体类型：纯元数据读取（PEReader），默认过滤编译器生成类型，无需缓存与超时。
    /// </summary>
    ...
    [McpServerTool]
    [Description("列出 .NET 程序集（dll/exe）中指定类别的实体类型，纯元数据秒回、默认过滤编译器生成类型（async 状态机、显示类等）。输出每行带行号标注，可直接引用具体行。结果默认只返回前 200 行，可用 lines 参数按行号范围拉取后续。")]
