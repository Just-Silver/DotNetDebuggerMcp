...
public static class DecompileToDirTool
{
    ...
    [McpServerTool]
    [Description("将 .NET 程序集（dll/exe）反编译写入指定目录（全量或指定单个类型）。结果写入磁盘而非标准输出，不做输出量截断；写盘完成后可直接读取输出目录下的源码文件。单文件输出（程序集或类型各一个 .decompiled.cs 文件）；按命名空间嵌套目录输出请使用 decompile_to_project。typeName 指定则仅反编译该类型，省略则反编译整个程序集。")]
