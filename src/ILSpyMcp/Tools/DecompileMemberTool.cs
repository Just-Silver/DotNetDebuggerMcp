...
public static class DecompileMemberTool
{
    ...
    [McpServerTool]
    [Description("反编译指定类型内单个或多个成员的实现体到标准输出（成员级入口，如某个方法体；整类型源码请用 decompile 工具）。按 memberName 子串搜索成员（忽略大小写，适合只知道方法名、不知道完整文档 ID 的场景；默认排除属性/事件访问器）。提供 typeName 时在指定类型内定位；省略 typeName 时跨程序集搜索全部类型（#MEMBER 分隔行带 type 字段标注成员所属类型，供分辨同名成员归属）。定位到多个成员时全部反编译并合并输出，行号连续，各成员前有 #MEMBER JSON 结构化分隔行（格式 #MEMBER {\"name\":\"...\",\"token\":\"0x...\",\"type\":\"...\"}，token 可直接用于后续反编译）；超过 20 个时仅返回成员签名清单（每行 #MEMBER JSON，含 name/token/signature/type）不反编译。提供 token 参数时直接按元数据 token 反编译单个成员（忽略 memberName，typeName 可不填，清单与分隔行中的 token 均可直接用）。结果默认只返回前约 8 KB，可用 lines 参数分页（超限签名清单同样支持）；无匹配时返回相近成员名提示。")]
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (matches.Count == 0)
        {
            var message = crossAssembly
                ...
                : $"类型 {typeName} 中未找到名称包含 \"{memberName}\" 的成员";
        ...
        var context = new FormatContext(assemblyFull, crossAssembly
