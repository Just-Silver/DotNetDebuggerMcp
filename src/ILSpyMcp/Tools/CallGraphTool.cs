...
public static class CallGraphTool
{
    ...
    [McpServerTool]
    [Description("查询 .NET 程序集（dll/exe）中指定类型的方法体调用关系：扫描其全部方法体的调用指令，输出被调用的程序集内部类型（跨程序集类型不计、编译器生成类型不计），以及程序集内哪些类型的方法体调用了它（反向扫描全部类型）。与 dependencies 的成员签名引用不同，本工具反映的是执行流中的实际调用。typeName 为类型全名，格式与 list_types 输出一致，可直接复制使用。结果可能为空（某段无调用时输出（无）占位）；大型程序集反向扫描可能较慢。结果默认只返回前约 8 KB，可用 lines 参数按行号范围拉取后续。")]
