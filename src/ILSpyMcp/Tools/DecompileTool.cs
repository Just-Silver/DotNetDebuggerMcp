...
public static class DecompileTool
{
    ...
    [McpServerTool]
    [Description("反编译 .NET 程序集（dll/exe）中指定类型的完整源码到标准输出（类型级入口，输出整个类型含全部成员，不按单个成员反编译）。输出每行带行号标注，可直接引用具体行。结果默认只返回前约 8 KB，超过时可用 lines 参数按行号范围拉取后续。只反编译单个成员（如某个方法体）请用 decompile_member 工具；全量/项目反编译请用 decompile_to_dir 工具。")]
