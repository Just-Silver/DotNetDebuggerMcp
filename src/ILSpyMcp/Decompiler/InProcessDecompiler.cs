...
{
    /// <summary>
    /// 超时包装：同步 work 放入 Task.Run 在后台执行；timeout 内未完成（或取消触发）时返回 timeoutHint（后台任务跑完即弃），
    /// 否则返回 work 的结果。注意 Task.Delay(timeout, cancellationToken) 在取消触发时会以取消状态完成，统一视为超时处理。
    /// </summary>
    /// <param name="work">要执行的同步反编译工作，返回文本。</param>
    /// <param name="timeout">最长等待时间；超时即返回 timeoutHint。</param>
    /// <param name="cancellationToken">取消令牌；取消触发同样返回 timeoutHint。</param>
    ...
    {
        return Execute(assemblyPath, (module, decompiler) =>
        {
    ...
    {
        return Execute(assemblyPath, (_, decompiler) => CheckOutputSize(decompiler.DecompileWholeModuleAsString()));
    }
    ...
    private static string Execute(string assemblyPath, Func<PEFile, CSharpDecompiler, string> work)
    {
        try
        {
            using var module = OpenModule(assemblyPath);
