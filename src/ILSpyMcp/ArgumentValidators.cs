...
public static class ArgumentValidators
{
    ...
    /// <summary>
    /// 校验反编译目标：typeName 与 member 只能指定其一。
    /// </summary>
    /// <param name="typeName">全限定类型名，缺省为空字符串。</param>
    /// <param name="member">成员文档 ID 或 token，缺省为空字符串。</param>
    /// <param name="error">校验失败时的错误提示；通过时为 null。</param>
    /// <returns>通过返回 true；失败返回 false 且 <paramref name="error"/> 非空。</returns>
    public static bool ValidateDecompileTarget(string typeName, string member, [NotNullWhen(false)] out string? error)
    {
        var hasTypeName = !string.IsNullOrWhiteSpace(typeName);
        var hasMember = !string.IsNullOrWhiteSpace(member);
        if (hasTypeName && hasMember)
        {
            error = "typeName 与 member 只能指定其一。";
            return false;
        }
        if (!hasTypeName && !hasMember)
        {
            error = "请指定 typeName 或 member 之一；全量反编译请使用 decompile_to_dir 工具并指定 outputDir。";
            return false;
        }
        error = null;
        return true;
