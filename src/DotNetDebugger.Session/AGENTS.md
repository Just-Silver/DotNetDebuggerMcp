...
- `Evaluation/` — **P6 表达式读值子集**：`ExpressionParser`（自研递归下降，不引 Roslyn——文法=规格，能解析即已支持；段数上限 `PathSegment.MaxSegments`）+ `ExpressionEvaluator`（`EvaluateCore` 同步核：字面量/`!`/比较逻辑单一来源，PathNode 经注入的 resolver 取值——外部门面投泵阻塞等待、P7 泵内直读；数值统一 decimal、字符串 Ordinal、布尔与 null 仅 ==/!=）+ `ExpressionConditionEvaluator`（**P7 条件断点求值器**：实现 Engine 定义的 `IBreakpointConditionEvaluator` 依赖倒置契约，`DebugSessionManager` 建会话时注入单例；红线=泵线程内纯计算+pathResolver 直读，绝不触碰 session/命令泵——再入死锁）。`ExpressionEvaluationException.Message` 即面向 agent 的中文提示（宿主直接展示）。
