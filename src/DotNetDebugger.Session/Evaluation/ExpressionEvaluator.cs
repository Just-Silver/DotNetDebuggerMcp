...
public static class ExpressionEvaluator
{
    ...
    public static Task<DebugEvalResult> EvaluateAsync(DebugSession session, int threadId, string expression, CancellationToken ct = default)
    {
        var ast = ExpressionParser.Parse(expression);
        // P7：统一走同步核——工具线程 ≠ 泵线程，阻塞等待投泵任务安全（EvaluatePathAsync 经命令泵执行）
        return Task.FromResult(EvaluateCore(ast, path => session.EvaluatePathAsync(threadId, path.Root, path.Segments, ct).GetAwaiter().GetResult()));
    }

    /// <summary>
    /// 同步求值核（P7 抽取，P6/P7 共用一份字面量/`!`/比较逻辑）：<paramref name="resolvePath"/> 提供 PathNode
    /// 的取值——泵内求值=引擎 ReadPathValue 直读（完全同步）；外部门面=投泵阻塞等待。
    /// resolvePath 须在当前线程同步返回（不得再入命令泵）。
    /// </summary>
    public static DebugEvalResult EvaluateCore(ExpressionNode node, Func<PathNode, DebugEvalResult> resolvePath)
    {
        switch (node)
        {
            ...
            case NotNode not:
            {
                var operand = EvaluateCore(not.Operand, resolvePath);
            ...
            {
                var left = EvaluateCore(compare.Left, resolvePath);
                var right = EvaluateCore(compare.Right, resolvePath);
    ...
    internal static string DescribeOperand(DebugEvalResult value) => value.Kind switch
    {
