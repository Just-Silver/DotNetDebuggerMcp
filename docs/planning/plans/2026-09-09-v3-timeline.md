...
```
5. 若 Session 库尚无 `[assembly: InternalsVisibleTo("DotNetDebugger.Session.Tests")]`，在 csproj/AssemblyInfo 补（internal `Feed` 接缝与测试同程序集可见）。
...
// 删除原 :126 的 Exited 处理器（原 [进程已退出 exitCode=N] 标记逻辑随之下移此处，避免双写重复行）；
// 输出读取（BeginOutputReadLine/BeginErrorReadLine 排空）保持原处。
process.Exited += (_, _) =>
{
