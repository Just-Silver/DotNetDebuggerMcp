...
| `2026-09-10-u1a-uia-only-ui-automation.md` | **U1A UI 自动化全 UIA 化 spec**（宿主 U1 改版）：弃用一切物理输入（删 `ui_invoke`/`ui_scroll`），改语义动词 `ui_action`（invoke/toggle/select/expand/collapse/focus/scroll/scrollintoview/windowstate）+ `ui_input` + `ui_get` + `ui_wait`（事件化）；不抢前台、必要时仅还原窗口；元素条件缓存+重解析、虚拟化实体化；并入 `debug_verify`（uiAction/uiAssert 步骤） | **已实施**（2026-09-10：宿主 `Services/Ui/` 组件拆分（facade + Locator/Dispatcher/StateReader/EventWaiter/UiModels）+ `ui_action`/`ui_input`/`ui_get` 工具（删 `ui_invoke`/`ui_scroll`）确定性护栏（宿主源码+程序集 IL 防物理输入扫描）与纯逻辑/解析测试全绿；Engine/Session 零改动；实施计划 `plans/2026-09-10-u1a-uia-only-ui-automation.md`）。**已由远程 CI（GitHub Actions windows-latest，run 34442360727）整跑 675/675 通过、0 警告签收；其间据 CI 实证修复 `ui_input` 假成功（写后读回确认、ValuePattern 静默 no-op 自动退 RangeValue、只读即终止）** |
...
| `2026-09-22-screenshot-tool-design.md` | **screenshot 截图工具 spec**（独立工具）：mode=window/screen/region 单工具多模式、WGC 优先+GDI 回退、CallToolResult image 块双轨、PackAsTool hack（spike 实证）、UiSampleApp 测试、Web 冻结不接入 | **已实现**（计划 `plans/2026-09-22-screenshot-tool.md` 执行完成 2026-09-22；三轮审查通过，第三轮 Approved） |
