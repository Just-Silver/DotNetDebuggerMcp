...
| `2026-09-07-r5-async-step.md` | **R5 async 状态机单步体验 spec**（agent 反馈 R5）：状态机帧识别 + step-over/out 自动连步滤帧 + step-into 诚实停住 + `debug_stack` 帧名真名化；参考 dnSpy DbgEngineStepperImpl | **草案待评审**（取舍点 §7） |
| `2026-09-07-r6-line-breakpoint-deferred.md` | **R6 行断点延迟解析登记 spec**（agent 反馈 R6）：引擎 LoadModule 发 ModuleLoaded 事件 + 宿主登记延迟项 + 模块加载自动补设 | **草案待评审**（取舍点 §7） |
