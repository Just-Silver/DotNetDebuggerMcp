...
| `2026-09-10-u1a-uia-only-ui-automation.md` | **U1A UI 自动化全 UIA 化 spec**（宿主 U1 改版）：弃用一切物理输入（删 `ui_invoke`/`ui_scroll`），改语义动词 `ui_action`（invoke/toggle/select/expand/collapse/focus/scroll/scrollintoview/windowstate）+ `ui_input` + `ui_get` + `ui_wait`（事件化）；不抢前台、必要时仅还原窗口；元素条件缓存+重解析、虚拟化实体化；并入 `debug_verify`（uiAction/uiAssert 步骤） | **已实施**（2026-09-10：宿主 `Services/Ui/` 组件拆分（facade + Locator/Dispatcher/StateReader/EventWaiter/UiModels）+ `ui_action`/`ui_input`/`ui_get` 工具（删 `ui_invoke`/`ui_scroll`）+ `ui_find` 能力清单 + `ui_wait` 事件化 + UiSampleApp 扩展 + DebugUiToolsTests UIA-only 重写 + `debug_verify` uiAction/uiAssert；`git grep FlaUI.Core.Input` 为空；确定性护栏（宿主源码+程序集 IL 防物理输入扫描）与纯逻辑/解析测试全绿；Engine/Session 零改动；实施计划 `plans/2026-09-10-u1a-uia-only-ui-automation.md`）。**注意：UI 端到端用例（`DebugUiToolsTests`/`DebugVerifyToolTests` UI 例）因开发机 UIA 全局 5s 超时未能在本机复现「隔离绿」，待安静/非交互环境整跑复验后签收**（bug 修复波 + re-review Approved，见计划/报告） |
