...
非目标：目标自身控制台输出（debug_output/timeline log 行）v1 不脱敏（目标自打内容，整行替换破坏调试）；~~异常 Message 不脱敏~~（**2026-09-10 修订**：停点 `message` 改为按内容形态脱敏——普通消息原样、含凭据形态给占位符，见 decisions.md D26；Web/CLI `-dbg` 仍不脱敏）；Web 监视器展示不脱敏（本地人类观看席，2026-09-09 拍板）；Engine/Session 层不脱敏（渲染时脱敏，纯能力层不感知展示策略）。
