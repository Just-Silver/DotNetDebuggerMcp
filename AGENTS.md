...
- 纯元数据工具（list_types/signature/hierarchy/dependencies）头部同样带信息块（IsListing：`匹配实体: N 个 + 总行数: N 行`，每行一个实体），默认返回前 200 行、同样支持 `lines` 分页。反编译输出含 `//IL_` 未解析注释时，头部追加「提示: 输出含 //IL_ 未解析注释（动态类型/异常路径），仅供结构参考」
