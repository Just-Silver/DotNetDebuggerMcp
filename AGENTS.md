...
在开发过程中，凡遇到以下情形，**严禁凭经验臆测或盲目修改代码**：
- 不熟悉某函数/API 的用法、参数或返回值；
- 不清楚某模块、框架或底层库的实现原理；
- 遇到报错、异常行为或性能问题，且原因不明；
...
**目标**：保证每次修改都有依据，降低调试成本，提升代码质量与团队协作效率。
...
解决方案为 5 程序集（`DotNetDebuggerMcp.slnx`），宿主 exe + 能力库分层，进程合一：

- `src/DotNetDebugger.Decompiler/` — **反编译/静态分析能力库**（`DotNetDebugger.Decompiler`，net10.0，仅依赖 ICSharpCode.Decompiler；无 MCP/DI/日志依赖）。源码按功能划分命名空间：`Configuration/`（`DotNetDebugger.Decompiler.Configuration`：`DecompilerConfig` 能力库常量 + `DecompilerText` 反编译失败前缀——宿主经 `InternalsVisibleTo` 转发引用，单一来源）、`Metadata/`（`DotNetDebugger.Decompiler.Metadata`，纯元数据读取组件）、`Decompiler/`（`DotNetDebugger.Decompiler.Decompiler`，`InProcessDecompiler.cs`）
- `src/DotNetDebugger.Engine/` / `src/DotNetDebugger.Session/` / `src/DotNetDebugger.Web/` — **预留占位库**（P2/P3/P4 分别实现动态调试引擎 / 会话服务 / WebUI，当前仅占位）
- `src/DotNetDebuggerMcp/` — **宿主 exe**（`DotNetDebuggerMcp`，net10.0、PackAsTool、框架依赖；NuGet 包 id / CLI 命令 `dotnet-debugger-mcp`；运行期需 .NET 10 运行时），含全部反编译 MCP 工具与执行管线。源码按功能划分命名空间（每目录一个）
  - `Tools/`（`DotNetDebuggerMcp.Tools`）— 11 个 `[McpServerToolType]` 静态类，分三类：**反编译类**（DecompileTool / DecompileMemberTool / DecompileToDirTool / DecompileToProjectTool，经 `InProcessDecompiler` 进程内反编译，带 `timeoutSeconds` 默认 30s——超时/取消按「放弃等待」处理：返回提示、不入缓存、可调大重试）与**纯元数据类**（ListTypesTool / SignatureTool / HierarchyTool / DependenciesTool / CallGraphTool / AssemblyInfoTool，秒回、无 timeoutSeconds、带 `lines` 分页参数，结果经共享缓存（`ToolExecutor.RunMetadata`），命中时头部标注「缓存: 命中」、未命中回源后入缓存，错误/未找到提示不入缓存；`list_types` 另有 `nameContains` 名称子串过滤参数，忽略大小写、默认空=不过滤）与**缓存观察工具**（CacheStatsTool，`cache_stats` 无程序集参数：输出共享缓存当前占用/上限、条目数、命中率与逐条目占用明细，供评估缓存大小设置）。**执行样板统一走 `ToolExecutor`**（`ResolveAssembly`/`RunPipelineAsync`/`RunMergedAsync`/`RunMetadata`）。`decompile` 仅类型级反编译（`typeName` 必填，成员级由 decompile_member 承接）；`decompile_member` 按成员名子串搜索（纯元数据定位，token 全局唯一；默认排除访问器；多匹配合并输出且各成员前插 `#MEMBER {"name","token"}` JSON 分隔行；匹配数 > `AppConfig.MaxMemberMatches`（20）仅返回 `#MEMBER` 签名清单不反编译；提供 `token` 参数时按 token 直接反编译单个成员（忽略 memberName，typeName 可不填）；无匹配返回相近成员名提示）；`to_dir` 的 `typeName` 非空时仅反编译指定类型，支持逗号分隔多个类型批量写盘（每类型一个 `{TypeName}.decompiled.cs`，未找到的附「未找到：」提示、部分成功也算成功），省略全量（恒单文件写盘，嵌套目录输出请用 `to_project`）；`to_project` 恒项目形式；`signature`/`hierarchy`/`dependencies`/`call_graph` 输出经元数据组件读取；`hierarchy` 另有 `includeIndirect` 参数（默认 false），为 true 时后代段一次返回接口/基类的全部间接后代，`dependencies`/`call_graph` 另有 `includeExternal` 参数（默认 false），为 true 时追加输出跨程序集外部类型引用（带程序集归属）
  - `Services/`（`DotNetDebuggerMcp.Services`）— `AppServices.cs` 进程级共享单例（缓存、执行管道、进程内反编译服务、NuGet 查询、更新检查报告缓存 `StatusReport`），避免各工具独立持有实例；测试经 `ConfigureForTest`/`ResetForTest` 注入 fake。**本层不得反向引用 `DotNetDebuggerMcp.Tools`**（交叉依赖已消除）。`ToolExecutor.cs` 工具执行共享辅助（路径安全解析 + 管道调用样板 + 元数据共享缓存辅助 `RunMetadata`，移入本层以消除 Pipeline→Services 循环）。`CheckTool.cs` 更新检查入口（**非 MCP 工具**——握手期已把报告注入 ServerInstructions，仅供 CLI `-c/--check` 调试）：报告当前 dotnet-debugger-mcp 是否有新版本；**结果会话内缓存**（重复检查无意义），同步读磁盘缓存、无有效检查记录时返回空报告
  - `Pipeline/`（`DotNetDebuggerMcp.Pipeline`）— `ToolPipeline.cs` 共享执行管道：缓存命中 → 进程内反编译回源（同 key 并发单飞）→ lines 分页格式化；回源经 `InProcessDecompiler.RunWithTimeoutAsync`，超时/取消放弃等待、错误提示不入缓存（同 key 可重试）。`ExecuteMergedAsync` 合并多条命令（decompile_member 多匹配）为一个大行列表后统一格式化。**`ToolCommand` 持有 `Assembly` 属性（程序集唯一数据源），`ExecuteAsync(ToolCommand command, ...)` 不再单独传 assembly——勿再造双份程序集参数**
  - `Caching/`（`DotNetDebuggerMcp.Caching`）— `DecompileCache.cs` 线程安全 LRU 缓存（默认 64MB，固定 30 分钟滑动过期 + 5 分钟定时清理，Get 命中刷新过期时间，结构化 CacheKey 含程序集指纹，dll 更新自动失效；反编译与元数据工具共用同一实例）
  - `Formatting/`（`DotNetDebuggerMcp.Formatting`）— `OutputFormatter.cs` 行号标注与 `lines` 分页
  - `UpdateCheck/`（`DotNetDebuggerMcp.UpdateCheck`）— `NuGetClient.cs` NuGet 最新稳定版查询（排除预发布，网络失败返回 null 供更新检查静默跳过）；`UpdateChecker.cs` NuGet 新版本检查的磁盘缓存与报告段组装（成功 TTL 24h、失败 1h 退避、失败保留旧值，落盘 `%LOCALAPPDATA%\dotnet-debugger-mcp\update-check.json` 跨进程共享；**查询经构造注入的委托 `queryLatest`**，生产由 AppServices 传共享 NuGetClient、测试传 fake，不反引用 Services 层）；`EnvironmentChecker.cs` 更新检查状态组装（仅 dotnet-debugger-mcp 更新状态，依赖经参数传入 updater，不反引用 Services 层）。状态经 `GetCachedNuGetStatus` 同步读缓存（零网络，返回 `NuGetUpdateStatus?` 含新版本标记与报告行，无有效检查记录时返回 null），**网络刷新由握手后台 `RefreshIfStaleAsync` 承担**（TTL/退避内不联网，失败静默降级；CLI `-c` 是主动调试入口，调用前先 await 刷新）；CLI `-c` 与握手注入分别组装文本——CLI 取状态行（`GetCachedNuGetLine`，供人阅读），握手注入走 `BuildHandshakeText`（有新版本时前缀明确指令要求 agent 会话开始主动告知用户并提供升级命令，已是最新仅状态行，无记录不注入）；**版本比较共用 `IsNewerThanCurrent` 静态方法**（报告与握手注入两处调同一规则，防漂移），当前版本统一取 `AppConfig.CurrentVersion`
  - `Configuration/`（`DotNetDebuggerMcp.Configuration`）— `AppConfig.cs` 全局配置常量（缓存/超时/匹配上限/NuGet 检查等可调参数集中维护；已入库的 ExternalExpandMax*/MaxOutputBytes 等不在此重复，见 Decompiler 库 DecompilerConfig）+ `AppText.cs`（用户可见文案，`DecompileFailurePrefix`/`StartsWithDecompileFailure` 转发 Decompiler 库 `DecompilerText` 单一来源）
  - `Validation/`（`DotNetDebuggerMcp.Validation`）— `ArgumentValidators.cs` 共享参数校验（assembly/必填/memberName/token/list/outputDir/timeoutSeconds，无安装前置）
  - MCP 握手期**注入 Markdown 功能简介**（`AppText.HandshakeFeatureIntro`：「## 服务器简介」、「## 何时使用」（**触发条件导向**——反编译/静态分析 与 动态调试 两类场景何时用，**不逐条列工具**——agent 经 MCP 工具目录发现）、「## 使用约定」三块标题分节；不再注入工作目录行——客户端系统环境已提供）；随后**先执行更新检查**（`await AppServices.StatusReport.Value` 拿到 `NuGetUpdateStatus?`，与 CLI `-c` 同源会话内缓存），经 `EnvironmentChecker.BuildHandshakeText` 组装为「## 更新状态」段（自带标题、状态行与 CLI 共用来源不在其内改写）附于功能简介之后一并注入 `McpServerOptions.ServerInstructions`：有新版本时注入指令式文本（要求 agent 在会话开始的第一条回复中主动告知用户并提供升级命令，陈述句会被 agent 当背景信息而不转述）；已是最新仅注入状态行；无有效检查记录时整段不注入、仅功能简介；`StatusReport.Value` 包 try/catch 降级为空报告，更新检查异常不阻断 MCP 启动
- `src/DotNetDebuggerMcp.Client/` — 端到端验证客户端：场景拆分为 `DecompileCases` / `DecompileMemberCases` / `ListTypesCases` / `DecompileToDirCases` / `SignatureCases` / `HierarchyCases` / `DependenciesCases` / `CallGraphCases`（各工具全参数覆盖）与 `ClientRunner`（连接/执行/输出）、`TestDataHelper`（自动发现测试 dll 并共享类型/成员标识），`Program.cs` 仅做入口
- `tests/DotNetDebuggerMcp.Tests/` — xUnit 单元测试（缓存/管道/格式化/校验/进程内反编译/元数据组件/更新检查，经 `TestDataPaths` 与测试程序集验证）
...
> Decompiler 库内的 `Metadata/` 与 `Decompiler/` 两段能力详情（各组件职责/边界）见上库条目与源码注释；宿主 `Tools/` 经 `using DotNetDebugger.Decompiler.Metadata;` 跨程序集引用库内类型（库对宿主 `InternalsVisibleTo`，宿主可访问库 internal 如 `InProcessDecompiler.IsErrorResult`）。

## 命令

```bash
dotnet build -c Release src/DotNetDebuggerMcp/DotNetDebuggerMcp.csproj
dotnet test --project tests/DotNetDebuggerMcp.Tests/DotNetDebuggerMcp.Tests.csproj   # 单元测试（xunit.v3 + MTP 模式，global.json 已配置 test.runner）
dotnet test --project tests/DotNetDebuggerMcp.Tests/DotNetDebuggerMcp.Tests.csproj -- --filter-class "DotNetDebuggerMcp.Tests.DecompileCacheTests"   # 只跑单套测试
dotnet run -c Release --project src/DotNetDebuggerMcp.Client/DotNetDebuggerMcp.Client.csproj   # 调全部工具做端到端验证
```

- Client 端到端会以 Release 自启动 server 项目（`dotnet run --project src/DotNetDebuggerMcp/DotNetDebuggerMcp.csproj -c Release`，无需预先单独构建 server），运行后自动清理写盘产物 `tests/.dotnetdebugger-client-out/`（已在 .gitignore）
- CLI 调试（改完 server 代码用 Debug 构建的 exe 快速验证，行为与 MCP 工具一致，是验证新行为的主要手段）：
  ```bash
  ./src/DotNetDebuggerMcp/bin/Debug/net10.0/DotNetDebuggerMcp.exe -a <dll> -t <TypeName>      # 反编译类型
  ./src/DotNetDebuggerMcp/bin/Debug/net10.0/DotNetDebuggerMcp.exe -a <dll> -t <TypeName> -mn <成员名子串>  # 按名搜成员（多匹配含 #MEMBER JSON 分隔行，>20 仅返回签名清单）
  ./src/DotNetDebuggerMcp/bin/Debug/net10.0/DotNetDebuggerMcp.exe -a <dll> -t <TypeName> -s   # 成员签名（API 地图，元数据秒回）
  ./src/DotNetDebuggerMcp/bin/Debug/net10.0/DotNetDebuggerMcp.exe -a <dll> -t <TypeName> -hc  # 继承/接口关系（元数据秒回）
  ./src/DotNetDebuggerMcp/bin/Debug/net10.0/DotNetDebuggerMcp.exe -a <dll> -t <TypeName> -hc -i  # 继承/接口关系含全部间接后代（元数据秒回）
  ./src/DotNetDebuggerMcp/bin/Debug/net10.0/DotNetDebuggerMcp.exe -a <dll> -t <TypeName> -d   # 成员签名内部引用（元数据秒回）
  ./src/DotNetDebuggerMcp/bin/Debug/net10.0/DotNetDebuggerMcp.exe -a <dll> -t <TypeName> -d -x  # 成员签名引用含跨程序集外部类型（元数据秒回）
  ./src/DotNetDebuggerMcp/bin/Debug/net10.0/DotNetDebuggerMcp.exe -a <dll> -t <TypeName> -cg  # 方法体调用关系（元数据秒回）
  ./src/DotNetDebuggerMcp/bin/Debug/net10.0/DotNetDebuggerMcp.exe -a <dll> -t <TypeName> -cg -x  # 方法体调用含跨程序集外部类型（元数据秒回）
  ./src/DotNetDebuggerMcp/bin/Debug/net10.0/DotNetDebuggerMcp.exe -a <dll> -l c               # 列 class（c/i/s/d/e 可组合，过滤编译器生成类型）
  ./src/DotNetDebuggerMcp/bin/Debug/net10.0/DotNetDebuggerMcp.exe -a <dll> -l c -nc Box       # 列 class 且名称含 Box 的类型（忽略大小写，配合 -l）
  ./src/DotNetDebuggerMcp/bin/Debug/net10.0/DotNetDebuggerMcp.exe -a <dll> -l c -ns ILSpyMcp.Samples  # 列 class 且命名空间含 ILSpyMcp.Samples（忽略大小写，配合 -l）
  ./src/DotNetDebuggerMcp/bin/Debug/net10.0/DotNetDebuggerMcp.exe -a <dll> -ss <子串>         # 按字符串字面量子串反查成员（可选 -t 限定类型，元数据秒回）
  ./src/DotNetDebuggerMcp/bin/Debug/net10.0/DotNetDebuggerMcp.exe -a <dll> -fa -t <TypeName> -fn <字段名>  # 追踪字段读写点（可选 -tk 按字段 token，元数据秒回）
  ./src/DotNetDebuggerMcp/bin/Debug/net10.0/DotNetDebuggerMcp.exe -a <dll> -iu -t <InterfaceName>  # 接口实现者与调用点（-i 含全部间接实现者，元数据秒回）
  ./src/DotNetDebuggerMcp/bin/Debug/net10.0/DotNetDebuggerMcp.exe -a <dll> -gi -t <GenericTypeName>  # 泛型实例化使用点（元数据秒回）
  ./src/DotNetDebuggerMcp/bin/Debug/net10.0/DotNetDebuggerMcp.exe -a <dll> -cc -t <TypeName> -mn <方法名>  # 起始方法调用序列 + 被调成员反编译（-tk 按方法 token 定位；-x 展开跨程序集外部调用）
  ./src/DotNetDebuggerMcp/bin/Debug/net10.0/DotNetDebuggerMcp.exe -a <dll> -t <TypeName> -mn <成员名> -tt <typeToken>  # typeName 有歧义时按类型 token（0x02）精确定位类型后搜成员
  ./src/DotNetDebuggerMcp/bin/Debug/net10.0/DotNetDebuggerMcp.exe -a <dll> -o <dir>           # 全量写盘；-p 组合为项目形式（to_project）
  ./src/DotNetDebuggerMcp/bin/Debug/net10.0/DotNetDebuggerMcp.exe -c                         # 检查 dotnet-debugger-mcp 是否有新版本（无需 -a）
  ```
  其他选项：`-ln start-end` 按行分页、`--timeout` 秒数
- 反编译引擎（ICSharpCode.Decompiler）随 NuGet 包内置，安装 dotnet-debugger-mcp 后开箱即用，无需额外安装反编译工具
...
- 本地调试注意：根 `opencode.json` 把本仓库自身的 MCP server 绑定到 `src/DotNetDebuggerMcp/bin/Debug/net10.0/DotNetDebuggerMcp.exe`，改完 server 代码需重新 build 并重启 opencode 才生效；会话内 `dotnetdebugger_*` 工具反映旧二进制，验证新行为请以 Client 输出为准
...
- 单测里经 `ToolPipeline` 的 assembly 路径解析基准是测试进程 CWD（`bin/Debug/net10.0`）；访问 `tests/TestData` 下 dll 用 `TestDataPaths.TestSamplesDll` 帮助类（`tests/DotNetDebuggerMcp.Tests/TestDataPaths.cs`，逐级上溯找 `DotNetDebuggerMcp.slnx`），MemberResolver 单测则直接用 `typeof(OutputFormatter).Assembly.Location`（主项目程序集，无需 TestData）
