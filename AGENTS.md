...
- `decompile_member` 提供 `token` 参数时按 token 直接反编译单个成员，头部目标描述为 `类型 X 的成员 <token>（按 token 反编译）`（typeName 缺省时为 `成员 <token>（按 token 反编译）`）；按名搜索时头部目标描述为 `类型 X 的成员 <memberName>（N 个匹配）`；多成员匹配合并输出（行号连续、总行数基于合并结果，各成员前有 `=== 名字 (token) ===` 分隔行、计入行号），匹配数 > `AppConfig.MaxMemberMatches`（20）时头部注明「超过上限，仅列出签名」并只返回签名清单（每行 `签名  [token]`）；无匹配返回「类型 X 中未找到名称包含 Y 的成员」、存在相近名时追加「相近成员：A、B、C」；类型不存在返回「未找到类型 X」
