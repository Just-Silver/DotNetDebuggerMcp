...
- 本地调试注意：根 `opencode.json` 把本仓库自身的 MCP server 绑定到 `src/DotNetDebuggerMcp/bin/Debug/net10.0-windows10.0.22621.0/DotNetDebuggerMcp.exe`（带 `--web` 仅为联调）——**改完 server 代码需重新 build 并重启 opencode 才生效**；该进程运行时会锁定 Debug 输出文件导致 Debug 构建失败（MSB3021，会话内可见）。验证新行为以 Client/CLI 输出为准。
- 单测里经 `ToolPipeline` 的 assembly 路径解析基准是测试进程 CWD（`bin/Debug/net10.0-windows10.0.22621.0`）；访问 `tests/TestData` 下 dll 用各测试项目 `TestDataPaths`/`TestPaths` 帮助类（上溯找 `DotNetDebuggerMcp.slnx`）。
