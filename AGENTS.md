...
- **CHANGELOG 只写使用者（agent/CLI）可见的变更**（新功能、行为变化、破坏性变更、可感知的修复、默认值/参数描述变化）——它是 NuGet `PackageReleaseNotes` 的展示文案，不是开发流水账；内部重构、实现细节、测试改动等一律不写（git 提交记录已足够）。某版本只有内部变更时写一行「内部重构与细节调整，无用户可见变化」占位，保住 CI「缺段即失败」的防静默机制。
...
- `src/ILSpyMcp/` — MCP 服务器（net10.0、PackAsTool、框架依赖；运行期需 .NET 10 运行时），源码按功能划分命名空间（每目录一个）
  ...
  - `Metadata/`（`ILSpyMcp.Metadata`）— 纯元数据读取组件（PEReader+MetadataReader，不加载程序集、不反编译 IL）：`MetadataNaming.cs`（类型全名渲染/定位，格式与 ilspycmd `-l` 对齐：命名空间.类型、嵌套用 `+`、泛型带 arity 如 `GenericBox\`1`；定位时 `+`/`.` 分隔均接受）、`CompilerGeneratedFilter.cs`（名含 `<` 即编译器生成类型；**刻意不用 `__` 前缀/特性兜底**——`__ComObject` 是合法类型、顶层语句 `Program` 带 CompilerGeneratedAttribute 但非编译器产物）、`SignatureRenderer.cs`（成员签名渲染，含 `RenderMemberSignature` 单成员）、`TypeLister.cs`（按类别枚举+过滤）、`Hierarchy.cs`（基类链/接口/后代）、`ReferenceExtractor.cs`（成员签名内部类型引用）、`MemberResolver.cs`（成员名子串搜索，返回 `MemberSearchResult` 含相近名；token `0x06000005` 直用于 `-m`）
...
- `tests/TestData/` — 验证用程序集（生成的 `ILSpyMcp.TestSamples.dll`：610 class + 1 interface，list_types 默认过滤 `<Module>` 等编译器生成类型；`Class0001-0600` 触发 200 行默认截断与 500 行分页上限；`BigClass` 含 BigMethod 600+ 行与 BigHelper/BigHelper2，触发 decompile 截断/分页与 decompile_member 多匹配；`BaseClass/DerivedClass/DerivedClass2` 与 `IAnimal/Dog` 供 hierarchy，`GenericBox\`1` 供泛型签名，`Members` 供属性/事件/访问器排除，`Uses` 供 dependencies，`WithClosure/WithAsync` 触发编译器生成类型；dll 与生成脚本 `generate-testdata.ps1` 入库，可重新生成；Client 经 `TestDataHelper` 自动发现目录下 dll 并对全部工具做端到端验证）
